...
public class Controller01 {
    ...
    private void runtareas(ActionEvent event) {
        ...
        if (sourceButton == boton1) {
            toggleTask(0, boton1, tarea1, runningtarea1);
        } else if (sourceButton == boton2) {
            toggleTask(1, boton2, tarea2, runningtarea2);
        } else if (sourceButton == boton3) {
            toggleTask(2, boton3, tarea3, runningtarea3);
        }
    }

    private void toggleTask(int index, Button button, Future<?> task, boolean running) {
        if (running) {
            task.cancel(true);
            button.setText("Iniciar");
        } else {
            task = backgroundTask(index, progresoactual(index));
            button.setText("Stop");
        }
        updateRunningStatus(index, button);
    }
    ...
    private void updateRunningStatus(int index, Button button) {
        switch (index) {
            case 0:
                runningtarea1 = !runningtarea1;
                break;
            case 1:
                runningtarea2 = !runningtarea2;
                break;
            case 2:
                runningtarea3 = !runningtarea3;
                break;
        }
    }
    ...
    private void checkAndStop(int index) {
        if (progresoactual(index) == 99) {
            stopExecutor();
            updateRunningStatus(index, null);
        }
    }
